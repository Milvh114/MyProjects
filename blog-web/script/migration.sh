...
if [ $1 == "generate" ]; then
    npm run typeorm migration:generate ./libs/database-type-orm/src/migrations/$2 -- -d ./libs/database-type-orm/src/data-source.ts --pretty --timestamp $MIGRATION_TIMESTAMP
